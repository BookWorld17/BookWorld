...
public class interestActivity extends BaseActivity {
    ...
    public tagsAdapter adapterTags;
    public static ArrayList<Tags> listInterest = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.buyer_intrest_tags, contentFrameLayout);
        ...
        Tags all = new Tags("0", "All",  false);
        listInterest.add(all);
        for(int i = 0; i<URLs.tegsList.size(); i++)
        {
            Tags c = URLs.tegsList.get(i);
            listInterest.add(new Tags(c.getId(), c.getName(), false));
        }
        adapterTags = new tagsAdapter(listInterest, this);

        rv_label.setHasFixedSize(true);
        rv_label.setLayoutManager(new GridLayoutManager(getApplicationContext(), 1));
        rv_label.setAdapter(adapterTags);
    ...
    private void doPostRequest() {
        ...
        for(int i = 0; i< listInterest.size(); i++)
        {
            if(listInterest.get(i).isSelected()){
                myInterests = myInterests + listInterest.get(i).getId() + ";";
            }
        }
    ...
    public void doAction(String response){
        ...
        try {
            ...
            if(success.equals("1")){
                alert.showAlertDialog(interestActivity.this,"",getString(R.string.Data_updated),true);
                Intent intent = new Intent(getApplicationContext(), BuyerMainActivity.class);
                startActivity(intent);
            }else{
                alert.showAlertDialog(interestActivity.this, "", getString(R.string.Could_not_updated_Data), false);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
