...
public class booksList extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        interface_title.setText(R.string.list_added_books);
        ...
        try {
            ...
            if( success.equals("1")){
                for(int i=1;i<users.length();i++){
                    ...
                    JSONArray tagsJSONlist = jo.getJSONArray("tags");
                    ArrayList<Tags> tagsList = new ArrayList<>();
                    for(int j=0; j<tagsJSONlist.length();j++){
                        JSONObject jsob = tagsJSONlist.getJSONObject(j);
                        String tag_id = jsob.getString("tag_id");
                        String tag_name = jsob.getString("name");
                        Tags t = new Tags(tag_id, tag_name, true  );
                        tagsList.add(t);
                    }
                    b.setBookTags(tagsList);
    ...
    public void delete(Book book) {
        ...
        alert.showMessageOKCancel(getString(R.string.confirm_book_delete)+book.getBookName()+"]? All copies book will be also deleted.", new DialogInterface.OnClickListener() {
            @Override
